...
# Backup DB FULL
expdp system/$OOS_AB_ORACLE_PASSWORD@$OOS_AB_ORACLE_SID full=Y dumpfile=DB_$NOW.dmp logfile=expdp_$NOW.log
...
### Create tmp dir ###                                                                                                                                                                           
mkdir $BACKUP/$NOW
$TAR -cf $BACKUP/$NOW/dmp.tar $DPDUMP_DIR/DB_$NOW.dmp
$TAR -cf $BACKUP/$NOW/log.tar $DPDUMP_DIR/expdp_$NOW.log
$TAR -cf $BACKUP/$NOW/app.tar $OOS_AB_BACKUP_DIR
